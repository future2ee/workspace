...
if(imageInput != null){ // 화면에 imageInput이 있을 경우
    ...
    imageInput.addEventListener("change", e => {
        ...
        if(file.size > maxSize){ // 선택된 파일의 크기가 최대 크기를 포과한 경우
    ...
    // X 버튼 클릭 시
    daleteImage.addEventListener("click", () => {
    ...
    document.getElementById("profileFrm").addEventListener("submit", e => {
        ...
        // 이전 프로필 이미지가 있다 -> 삭제
        if(initCheck == true && deleteCheck == 0) flag =false;



        // 이전 프로필 이미지가 있다 -> 새 이미지
        if(initCheck == true && deleteCheck == 1) flag =false;
