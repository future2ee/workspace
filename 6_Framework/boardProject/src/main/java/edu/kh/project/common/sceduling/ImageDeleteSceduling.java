package edu.kh.project.common.sceduling;
...
public class ImageDeleteSceduling {
	...
	public void test() {
		...
		// 서버에 저장된 파일 목록을 조회해사
		// DB에 저장된 파일을 목록과 비교하여
		// 매칭되지 않는  서버 파일 제거
		
		// 1) 서버에 저장된 파일 목록 조회
		// -> application 객체를 이용해서
		// 	  /resources/images/board의 실제 서버 경로를 이용해서
		...
		if(!serverImageList.isEmpty() ) {
			...
			for(File server : serverImageList) {
				...
				// List.indexOf(객체) = 객체가 List에 있으면 해당 인덱스를 반환
				//					   없으면 -1 반환
