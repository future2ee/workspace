...
-- [관리자 계정] member 계정 생성
CREATE USER lmr_member IDENTIFIED BY member1234;

-- [관리자 계정] CONNECT, RESOURCE + CREATE VIEW 권한 부여 + 객체 생성 공간 할당
GRANT CONNECT, RESOURCE, CREATE VIEW TO lmr_member;
ALTER USER lmr_member DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;

-- lmr_member 계정 접속 방법 추가
...
COMMENT ON COLUMN MEMBER.SECESSION_FL IS '회원번호(PK)';
...
COMMENT ON COLUMN REPLY.CREARE_DATE IS '댓글 작성일';
...
ALTER TABLE MEMBER DROP(MEMBER_DATE);
...
ALTER TABLE REPLY ADD FOREIGN KEY (BOARD_NO)REFERENCES BOARD(BOARD_NO)ON DELETE SET NULL;
