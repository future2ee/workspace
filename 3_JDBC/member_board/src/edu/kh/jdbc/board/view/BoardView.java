...
public class BoardView {
	...
	/** 게시판 전용 메뉴 화면
	 * @param loginMember(로그인한 회우너 ㅓ정보)
	 */
		...
		do {
			...
			try {
	            ...
	            case 2 : selectOne(loginMember); break;
	            	// 상세 조회 시 게시글 수절/ 삭제 (게시글 작셩 == 로그인한 회원 비교)
	            	// 댓글(누가 작성? /작성자가 수정, 삭제 확인)
	            	// loginMember를 매개변수로 전달
	            case 3 : insertBoaard(loginMember.getMemberNo()); break;
	            case 4 : searchkBoard(); break;
	...
	private void insertBoaard(int memberNo) {
		System.out.println("\n[게시글 작성]\n");
	...
	private void selectOne(Member loginMember) {
		...
		try {
			...
			if(board != null) { // 조회된 게시글이 있을 경우
	            ...
	            System.out.println("1. 댓글 삽입"); // 어떤 회원ㅇㅣ든 가능
	            ...
	            // 댓글 번호 입력 -> 댓글이있는지 확인 -> 해당 댓글이 로그인한 회원꺼가 맞는지 확인
		...
		reply.setReplyContent(replyContent);
		  ...
		  try {
	         ...
	         if(result > 0) {
	            ...
	            System.out.println("댓글 수정이 완료되지 않았습니다.");
	         }
	 ...
	 * 						   // 생성된 객체에 "abc" 문자열 저장
	 * 
	 * 		str = "123";	// Heap영역에 새로운 String 객체가 생성되고
	 * 						// 생성된 객체에 "123" 문자열 저장 후
	 *						// 객체 주소를 str에 대입
	 * 
	 * ex) String str = "abc";
	 * 	str += "123";  // "123"이 저잔된 String 객체 생성 후
	 * 				   // "abc"와 "123"이 합쳐진 String 객체가 추가로 별도 생성
	 * 				   // 그 후 "abc123" 객체릐 주소를 str에 저장
	 * 
	 *  **String의 문제점**
	 *  
	 *  - String에 저장된 값ㅇ르 바꾸거나 누적하려고 할 때 마자
