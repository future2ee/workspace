...
public class MemberView { // 회원 관련 화면 입/출력
	...
	public Member login() {
		...
		try {
			...
			if(loginMember != null) { //참조하는 객체가 있다 == 로그인 성공
				System.out.println("\n ***"+ loginMember.getMemberName() +" 님 확영합니다.***\n");
	...
	public void updateMyInfo(Member loginMember) {
		...
		try {
			// UPDATE == DML == 수행 성공한 결과 행ㅇ의개수를 반환 == 정수형
			int result = service.updateMyInfo(updateMember);
			
			if(result > 0) { // 수정 성공
				// 수정 성공 + 수정한 값으로 로그인 멤버 바꾸기
	...
	public void updatePw(Member loginMember) {
		...
		System.out.println("[비림번호 변경]");
		// 현재 비밀번호 --> DB Dudate 조건(WHERE)
