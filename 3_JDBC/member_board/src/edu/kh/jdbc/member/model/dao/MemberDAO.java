...
//							 -> DB 접근할 수 있다 == SQL 수행하고 결과를 받환 받을 수 있다.

// Java에서 DB에 접근하고 결과를 반환받기 위한 프로그램밍 API를 제공함
...
public class MemberDAO {
	...
	public int duplicateCheck(Connection conn, String memberId) throws Exception {
		...
		try  {
			...
			// -> 아이디 중복 검사 SELECT 결과는 0또는 1이라는 1행 결과가 무조건 나옴
			// -> while 문보다  if문을 사용하는게 효율적
			
			if(rs.next()) {
				result = rs.getInt(1); //은 컬럼 순서
			}
	...
	public int updatePw(Connection conn, int memberNo, String currentPw, String newPw) throws Exception {
		...
		try {
			...
			//4) SQL(INSET) 수행후 결과(성공한 행의 개수) 반환 받기
	...
	public int secession(Connection conn, int memberNo, String memberPw) throws Exception {
		...
		try {
			String sql = prop.getProperty("seccesion");
