...
// ** input 이벤트 **
// -> 입력과 관련된 모든 동작(key 관련, mouse 관련, 붙여넣기)
memberTel.addEventListener("input",function(){
    ...
    if(regExp.test(memberTel.value)){ // 유효한 경우
        ...
        telMessage.classList.add("confirm"); // confirm 클래스 추가
        telMessage.classList.remove("error"); // error클래스 제거

        checkObj.memberTel = true; // 유효한 상태임을 기록

    } else{ //유효하지 않은 경우
        telMessage.innerText="전화번호 형식이 올바르지 않습니다."
        telMessage.classList.add("error"); // confirm 클래스 추가
        telMessage.classList.remove("confirm"); // error클래스 제거
...
memberEmail.addEventListener("input",function(){
    ...
    if(regExp.test(memberEmail.value)){ //유효한 경우
        ...
        $.ajax({
            ...
            // 상대 경로 : /community/member/signUp/emailDupCheck
