...
public class LoginServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// POST방식 요청시 문자 인코딩이 서버 기본값으로 지정
		// -> 한글 깨짐 -> 문자 인코딩 변경 필요
		...
		try {
			...
			// 클라이언트 요청 -> 서버 요청 처리(Servlet) -> 응답 화면 만들어줘(JSP)
			...
			// ew) 아.아 주세요	->	주문 받음		->   바리스타가 만든 커피
			//		 클				캐셔(Servlet)		   응답 결과(JSP)
			
			
			// 2. Redirect(재요청)
			// - 현재 Servlet에서 응답 페이지를 만들지 않고 
			//   응답 페이지를 만들 수 있는 다른 요청의 주소로 클라이언트를 이동 시킴(재요청)
			...
			// CGV 카페
			// 팝콘 주세요 		->		팝콘 파는 위치를 알려줌 	->   (클) 팝콘 파는 곳으로 이동		
