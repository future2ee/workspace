...
public class SelectOneServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			// 회원 정보 조회 Serevice 호출 후 결과(Member 객체) 반환 받기
			 ...
			 * 
			 * // JSONObj의 toString()메소드는 // JSON 형태로 출력될 수 있도록 오버라이딩이 되어있다.
			 * resp.getWriter().print(obj.toString());
			 * 
			 * 
			 * 
			 * }else { resp.getWriter().print(member); }
			 */
				
			// 3) GSON 라이브러리를 이용한 Java 객체 -> JSON 반환
			
			// newGson().toJson(객체, 응답 스크림);
			// -> 매개변수에 작성된 객체를 JSON 형태로 변환환 후 스트림을 통해서 출력
