...
// 컨트롤러 ㅣ 요청에 따라 알맞은 Service를 호출하고 결과에 따라 응답을 제어
...
public class BoardWriteController extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			// -> 객체가 생성됨과 동시에 파라미터로전달된 파일이 지정된 경로에 저장(업로드)된다!
			...
			while(files.hasMoreElements()) { //다음 요소가 있으면 true
				String name = files.nextElement(); // 다음 요소 (ndame 속성 값)를 앋아옴
				...
				if(rename != null){ // 업로든된 파일이 있을 경우 ==
									// 현재 files에서 얻어온 name 속성을 ㅣㅇ용해
									// 원본 또는 변경을 얻어왔을 때 그 값이 null이 아닌경우
					
					//이미지 정보를 담은 객치(BoardImage)를 생성
