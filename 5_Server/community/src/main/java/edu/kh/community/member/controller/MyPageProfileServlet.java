...
public class MyPageProfileServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			HttpSession session = req.getSession(); // session얻어오는 것은 지장 없음(사용 가능)
			
			// 최상위 경로("/" == webapp 폴더)의 컴퓨터상의 실제 절대 경로 얻어옴
			String root = session.getServletContext().getRealPath("/");
			
			// 실제 파일이 저장되는 폴더의 경로
			String folderPath = "/resources/images/memberProfile/";
			
			// memberProfile 폴더 까지릐 절대 경로
			...
			MultipartRequest mpReq = new MultipartRequest(req, filePath, maxSize, enncoding, new MyRenamePolicy());
			...
			// 2) 업로드된 프로필 이미지의 웹 접근 경로(fordlerPath + 변경된 파일 명)
			
			// getOriginalFileName("input type='file'의 name 속성")
			// -> 원본 파일명
			...
			// (주의) multipart/form - adata 형식의 요청을 처이 중이기 때문에
			//       req를 이용해서 파라미터를 얻어올 수없다!
			//       ->mpReq를 이용하면 가능!
			...
			// 2) delete의 값이 1(눌러진 경우) 이먄 profileImage의 값을 null로 변경
			if(delete == 1) {
				profileImage = null;
			}
			...
			if(result>0) { // 성공
				...
				session.setAttribute("message", "프로필 이미지 변경 실패 ㅠㅠ");
			}
