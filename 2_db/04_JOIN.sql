...
--> 연결되는 컴럼의 값이 일치하는 행들만 조인됨. ( = 일치하는 값이 없는 행은 조인에서 제외됨.)

-- 작성 방법 크게 ANSI 구문과 오라클 구문으로 나뉘고
-- ANSI에서 USING과 ON을 쓰는 방법으로 나뉜다.

-- * ANSI 표준 구문
-- ANSI는 미국 국립 표준 협회를 뜻함, 미국의 산업 표준을 제정하는 민간단체로
-- 국제표준화기구  ISO에 가입되어있다.
-- ANSI에서 제정한 표준을 ANSI라고 하고
-- 여기서 재정한 표준 중 가장 유명한 것이 ANSCII코드이다.
...
-- 서로 같은 부서 코드를 나타낸다.
--> 이를 통대 두 테이블이 관계가 있음을 알고 조인을 통해 데이터 추출이 가능
...
-- 오라클 방식 -> 별칭 사용
-- 테이블 별로 별칭을 등록할 수 있음
SELECT EMP_ID, EMP_NAME, E.JOB_CODE, JOB_NAME --동일한 컬럼에 별칭 사용(E나 J나 상관 X)
...
-- 두 테이블의 지정하는 컬럼 값이 일치하지 않는 행도 조인에 포함 시킴
--> * 반드시 OUTERJOIN임을 명시해야 한다.
...
--> JOIN 구문 기준으로 왼쪽에 작성된 테이블의 모든 행이 결과(RESULT DET)에 포함됨
...
--'='(등호)를 사용하지 않는 조인문
-- 지정한 컬럼 값이 일치하는 경우가 아닌, 값의 범위에 포함되는 행들ㅇ르 연결하는 방식
...
-- 반드시 두 테이블 간의 동일하 컬럼명, 타입을 가진 컬럼이 필요
--> 없을 경우 교차조인이 됨
