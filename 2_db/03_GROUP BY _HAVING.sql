...
--* GROUP BY 절 : 같은 값들이 여러개의 기록된 컬럼을 가지고
--                같은 값을들 하나의 그룹으로 묶음
-- GROUP BY 컬럼명 | 함수식,...
-- 여러개의 값을 묶어서 하나로 처리할 목적으로 사용함
-- 그룹으로 묶은 값에 대해서는 SELECT절에서 그룹 함수를 사용함

-- 그룹 함수는 단 한개의 결과 값만 산축하기 때문에 그룹이 여러 개일 경우 오류 발생
-- 여러 개의 결과 값을 산출하기 위해 그룹 함수가 적요된 그룹의 기준을 ODER BY 절에 기술하여 사용
...
-- EMPLOYEE 테이블에서
-- 부서 코드와 부서별 보너스를 받는 사원의 수를 조회하고
-- 부서코드 순으로 정렬
...
SELECT DEPT_CODE, FLOOR(AVG(SALARY))
FROM EMPLOYEE
WHERE SALARY >= 3000000 -- 급여가 300만 이상인 지원의 부서별 급여 평균
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE;
...
-- * 인자로 전달 받은 그룹 중에서 가장 먼저 지정한 그룹별 합계와 총 합계를 구하는 함수
...
-- * 그룹으로 지정도니 모든 그룹에 대한 집계와 총 합계를 구하는 함수
...
-- 여러가지 조건이 있을 때 그에 해당하는 여러대의 결과값을 결합시키고 싶을 때 사용
-- 초보자들이 사용하기 쉽다.
-- (주의) 집합 연산에 사용되는 SELECT문은 SELECT절이 동일해야함

-- UNION은 OR 같은 개념(합집합) --> 중복제거
