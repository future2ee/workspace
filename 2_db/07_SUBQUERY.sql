...
/* 서브 쿼리 유형
    ...
    - 스칼라 서브쿼리 : 상관 쿼리이면서 결과 값이 하나인 서브 쿼리
...
--1) 부서별 급여 합 중 가장 큰 값 조회 (부서별 급여합 1등)
...
--2) 직원의 사번, 이름, 부서명, 직급명 조회(부서명 없는 사람 포함)
...
-- 3-2) ANY를 이용하여과장 중 가장 급여가 적은 직원 초과하는 대리를 조회
SELECT EMP_ID, EMP_NAME, JOB_NAME, SALARY
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
WHERE JOB_NAME = '대리'
...
-- 1) LOCATION 테이블을 통해 NATIONNAL_CODE가 KO인 LOCAL_CODE 조회
SELECT LOCAL_CODE
FROM LOCATION
...
FROM EMPLOYEE
WHERE DEPT_CODE IN(SELECT DEPT_ID --다중행(IN 사용)
                   FROM DEPARTMENT
                   WHERE LOCATION_ID = (SELECT LOCAL_CODE
                                        FROM LOCATION
...
-- 사번, 이름, 직급, 급여를 조회하세요
-- 단 급여와 급여 평균은 만원 단위로 계산하게요 TRUNC(컬럼명, -4)
...
-- 상관 커리는 메인쿼리가 사용하는 테이블 값을 서브쿼리가 이용해서 결과를 만듦
...
--3) 서브쿼리 결과를 아용해 메인쿼리 해석 중인 1행을 대상으로 조회
...
-- 서브쿼리가 만든 결과의 집합(RESULT DET)을 테이블 대신 사용한다.
