...
--           디벨로퍼의 날자 표기 방법이 년/월/일 로 지정되어 있는 것이다.)
...
/* BETWEEN을 이용한 날짜 비교*/
-- EMPLOYEE 에티블에서
-- 입사일 1990101 ~ 1999/12/31/ (90년도 입사자)
-- 직원의 사번, 이름, 입사일 조회
...
'_' 예시
1) 'A_' : A 뒤에 하무거나 한 글자
2) '___A' : A 앞에 아무거나 세 글자( 4글자 문자열이면서 A로 끝나야함)
...
-- 해결 방법 : EXCAPE OPTION을 이용하여 일반 문자로 처리할'_','%' 앞에 아무 특수문자 붙임
SELECT EMP_ID, EMP_NAME, EMAIL
FROM EMPLOYEE
WHERE EMAIL LIKE '___#_%' ESCAPE '#' ;
                --> $ 뒤 한 글자만 (_)을 일반 문자 취금
...
--3. EMPLOYEE 테이블에서 메일 주소'_'의 앞이 4자 이면서 DEPT_CODE가 D9 또는 D6이고
--   고용일이 90/0101 ~ 00/12/01이고, 급여가 270만 이상인 사원의 전체를 조회
SELECT * FROM EMPLOYEE
...
-- SELECT문의 조회 결과( RESELT SET)를 정렬할 때 작성하는 구문

- ***** SELECT문 가장 마지막에 해석 *****

[작성법]

3 : SELECT 컬럼명 AS 별칭, 컬럼명, 컬럼명, 컬럼명....
1 : FROM 테이블명
2 : WHERE 조건식
...
--EMPLOYEE 테이블에서 급려 "오름차순"으로 이름, 급여 조회
SELECT EMP_NAME, SALARY
FROM EMPLOYEE
ORDER BY SALARY ASC; --(ASENDING : 오름차순)

--EMPLOYEE 테이블에서 급려 "내림차순"으로 이름, 급여 조회
SELECT EMP_NAME, SALARY
FROM EMPLOYEE
...
SELECT EMP_NAME, BONUS
FROM EMPLOYEE
-- ORDER BY BONUS; -- NULLS LAST 기본 값
-- ORDER BY BONUS DESC; -- NULLS FIRST가 기본 값
ORDER BY BONUS NULLS FIRST;

-- 오름차순 정렬 시 NULLS LAST가 기본 값
-- 내림차순 정렬 시 NULLS FIRST가 기본 값
