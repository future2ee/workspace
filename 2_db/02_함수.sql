...
-- 그룹(GROUP) GKATN : N개의 값을 읽어 1개의 결과 반환

--함수는 SELECT절, ORDER BY절, GROUP BY절, HAVING절 사용 가능
...
-- INSTR('문자열' | '컬럼명', '찾을문자', [찾을위치 시작위치],[순번])
-- 지정한 위치부터 지정한 순번째로 검색되는 문자의 시작 위치 반환

-- 문자열에서 맨 앞에 있는 B 위치 조회
SELECT INSTR('AABAACAABBAA', 'B') FROM DUAL;

-- 문자열에서 5번째 부터 검색해서 맨 앞에 있는 B위치 조회
...
-- EMPLOYEE 테이블에서 사원명, 이메일 중 아이디만 조회sun_di@or.kr
...
SELECT '    K  H    ', TRIM ('    K  H    ')FROM DUAL; -- 양쪽 공배 제거(중간 미포함)
...
SELECT 123.456, ROUND(132.456) FROM DUAL; --소수점 첫째 자리에서 반올림
...
SELECT (123.5), CELI(123.5), FLOOR(123.5) FROM DUAL;
...
-- MONTHS_BETWWEN (날짜, 날짜) : 두 날짜의 개월 수 차이 반환
...
-- EMPLOYEE 테이블에서 각 사원의 이름, 입사 년도, 일사 월, 입사 일 조회
SELECT EMP_NAME 이름, 
...
-- 0 : 숫지 힌 칸을 의미, 여러 개 작성 시 오른쪽 정렬 + 빈칸 0 추가
...
FROM EMPLOYEE;
--> 년, 월 ,일은 오라클에 등록된 날짜 표기 패턴이 아니라ㅓ 오류
...
SELECT TO_DATE('04103 143000', 'YYMMDD HH24MISS') FROM DUAL;

SELECT TO_CHAR(TO_DATE('04103 143000', 'YYMMDD HH24MISS'), 'YYYY/MM/DD HH24"시"MI"분"') FROM DUAL;
...
/* 숫자 형변환 */
-- TO_NUMBER(문자 데이터,[포멧]) : 문자형 데이터를 숫자 데이터로 변경
...
SELECT EMP_NAME 사원명, 
    FLOOR(ABS(HIRE_DATE - SYSDATE),0) "근무일수1",
    FLOOR(ABS(SYSDATE - HIRE_DATE),0) "근무일수2"
FROM EMPLOYEE;
...
SELECT EMP_NAME 직원명, JOB_CODE 직급코드,
    TO_CHAR((SALARY + NVL(BONUS,0)*SALARY)*12,'L999,999,999') AS"연봉(원)"
FROM EMPLOYEE;
...
-- MIN(컬럼명) : 최소값
-- MAX(컬럼명) : 최대값
--> 타입 제한이 없음( 숫자 : 대/소, 날짜 : 과거/미래, 문자열 : 문자 순서)
