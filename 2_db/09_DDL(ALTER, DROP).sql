...
-- DEPT_COPY 테이블에 LOCATION_ID 컬럼에 CHECK 제약조건 추가
...
-- NOT NULL을 제외한 제약 조건은 추가적린 조건으로 인식됨 (ADD/DROP)
...
ALTER TABLE DEPT_COPY DROP CONSTRAINT SYS_C007547; -- NOT NULL

-- NOT NULL 제거 시 MODIFY 사용
ALTER TABLE DEPT_COPY
MODIFY DEPT_TITLE CONSTRAINT SYS_C007548 NULL;
...
--          : ALTER TABLE 테이블명 MODIFY 컬럼명 데이터타입 '값'; (기본값 변경)
...
SELECT * FROM DEPT_COPY; -- > XNAME 컬러 ㅁ추가 확인

--(추가)
-- DEPT_COPY 테이블에 LNAME VARCHAR2(30) 기본값 '한국' 추가
ALTER TABLE DEPT_COPY 
ADD (LNAME VARCHAR2(30) DEFAULT '한국');

SELECT * FROM DEPT_COPY; --LNAME컬럼이 추가되어있고, 컬럼 값이 모두 '한국'으로 되어있음
-- (수정)
-- DPET_COPY 테이블의 DEPT_ID 컬럼의 데이터 타입을 CHAR(2)-> VARCHAR2(3)으로 변경

-- 컬럼 수정 : ALTER TABLE 테이블명 MODIFY 컬럼명 데이터타입; (데이터 타입 변경)
ALTER TABLE DEPT_COPY MODIFY DEPT_ID VARCHAR2(3);

--(수정 에러 상황)
-- DEPT_TITLE 컬럼의 데이터 타입으 VARCHAR2(10)으로 변경
...
-- (기본값 수정)
-- LNAME 기본값 '한국' -> '대한민국'으로 변경
-- ALTET TATBLE 테이블명 MODIFY 컬럼명 DEFAULT '값'
ALTER TABLE DEPT_COPY MODIFY LMANE DEFAULT '대한민국';

SELECT * FROM DEPT_COPY; -->기본값을 변경했다고 해서, 기존에 지정된 값이 변경되지 않음
...
ROLLBACK; -- 트랜잭션(DML을 이영한 데이터 변경 사항을 저장)을 삭제하고 마지막 커밋상태로 돌아감

SELECT * FROM DEPT_COPY;
-- ROLLBACK 안됨!
-- 왜? CREATE / ALTER / DROP 같은 DDL은 ROLLBACK의 대상이 아님

-- * DDL / DML을 혼용해서 사용할 경경우 발생하는 문제점
-- DML을 수행하여 트랜잭션에 변경사항이 저장된 상태에서
-- COMMIT / ROLLBACK 없이 DDL 구문을 수행학 되면
...
--> 다른 테이블이 TB1 테이블을 참조하고 잇어서 삭제 불가능

-- 해결 방법 1 : 자식 -> 부모 순서로 삭제
-- (참조하는 테이블이 없으면 삭제 가능)
DROP TABLE TB2;
...
--> 제약 조건까지 모두 삭제
-- == FK 제약 조건으로 인해 삭제가 원래는 불가능 하지만, 제약 조건을 없애버려서 FK 관계를 해제
...
SELECT * FROM DCOPY;
SELECT * FROM DEPT_COPY; -- 이름이 변경되어서 DEPT_COPY 테이블명으로 조회 불가
