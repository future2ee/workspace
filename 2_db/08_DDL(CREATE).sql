/*
- 데이터 직셔너리란?
지원을 효율적으로 관리하기 위한 다양한 정보를 저장하는 시스템 테이블
데이터 직셔너리는 사용자가 테이블을 생성하거나 사용자를 변경하는 등의
작업을 할 때 데이터베이스 서버에 의해 자동으로 갱신되는 테이블

- User_tables : 자신의 계정이 소유한 객체 등에 정보를 조회할 수 있는 딕셔너리 뷰
*/
...
-- MEMBER 테이블 생성
CREATE TABLE MEMBER(
    MEMBER_ID VARCHAR2(20), -- 가변길이 문자열 20 바이트(영어, 숫자만 작성시 20글자)
    MEMBER_PWD VARCHAR2(20),
    MEMBER_NAME VARCHAR2(30), --한글은 3BYTE * 10글자 == 30BYTE
...
-- 만든 테이블 확인
SELECT * FROM MEMBER;
SELECT * FROM USER_TABLES; -- 딕셔너리 뷰(데이터 딕셔너리에서 이부분만 뽑아서 만든 가상 테이블)
-- 접속창 -> 계정 열고 -> 테이블 클릭 -> 위쪽 새로고침 클릭 -> MEMBER 테이블 보임
...
INSERT INTO MEMBER2('김영희', 0011134567890);
SELECT * FROM MEMBER2;
--> NUMBER 타입 컬럼에 데이터 사입 시 앞에 0이 있으면 사라지는 문제점이 있음!!
...
-- USER_CONS_COLUMS : 제약조건이 걸려있는 컬럼을 확인하는 딕셔너리 뷰
...
-- ORA-01400: NULL을 ("LMR"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
...
CREATE TABLE USER_USED_UK(
    ...
    -- 테이블 레벨 제약조건 설정 부분
    --UNIQUE(USER_ID) -- 테이불 레벨 제약조건 설정( 제약조건명 미지정)
...
-- ORA-00001: 무결성 제약 조건(LMR.USER_ID_U)에 위배됩니다
            --> UNIQUE
...
-- ORA-00001: 무결성 제약 조건(LMR.USER_ID_NAME_U)에 위배됩니다
...
-- 기본키 중복으로 오류
-- ORA-00001: 무결성 제약 조건(LMR.USER_NO_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL이므로 오류
-- ORA-01400: NULL을 ("LMR"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
-- 회원 번호와 아이디 둘 다중복 되었을 때만 제약조건 위배 에러 발생
-- ORA-00001: 무결성 제약 조건(LMR.PK_USERNO_USERID)에 위배됩니다

SELECT * FROM USER_USED_PK2;

--PRIMARY KEYY는 NULL이 들어갈 수 없음
INSERT INTO USER_USED_PK2
...
CREATE TABLE USER_USED_FK(
 ...
 ,
  CONSTRAINT GRADE_CODE_FK FOREIGN KEY(GRADE_CODE) REFERNCES USER_GRADE
);
...
--> 50이라는 값은USER_GRADE 테이블 GRADE_CODE 컬럼에서 제공하는 값이 아니므로
-- 외래키 제약 조건에 위배되어 오류 발생
-- ORA-02291: 무결성 제약조건(LMR.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
                --> 참조 무결성 위배
...
WHERE GRADE_CODE = 10;
--ORA-02292: 무결성 제약조건(LMR.GRADE_CODE_FK)이 위배되었습니다- 자식 레코드가 발견되었습니다

-- 1) ON DELETE RESTRICTED( 삭제 제한 )로 기본 지정되어 있음
-- FOREIGN KEY로 지정된 컬럼에서 사용되고 있는 값일 경우
-- 제공하는 컬럼의 값은 삭제하지 못함

-- GRADE_CODE 중 20은 외래키로 참ㅁ조되고 있지 않으므로 삭제가 가능함
COMMIT;
...
-- 부모 테이블인 USER_GRADE2에서 GRADE = 10삭제
...
SELECT * FROM USER_GRADE3; -- 부모행 삭제 확인 (10 없어짐)
...
-- ORA-02290: 체크 제약조건(LMR.GENDER_CHECK)이 위배되었습니다
...
CREATE TABLE USER_TEST(
    ...
    PNO VARCHAR2(20) CONSTRAINT NN_PNO NOT NULL UNIQUE,
...
-- NOT NULL 제약 조건 복사된 EMPLOYEE_COPY 테이블에
