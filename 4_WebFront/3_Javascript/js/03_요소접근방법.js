...
function accessId(){
    ...
    // *** 자바 스크립트는 문자열 비교시에도 비교 연산자를 사용한다 !! ***
    if(bgColor == "red"){
...
function accessClass(){
    ...
    arr[2].style.backgroundColor="tomato";
...
function inputTest(){
    ...
    //    요소의 내용( 시작태그, 동료태그 사이에 작성된 내용)을
    //    얻어 오거나, 변경할 때만 사용 가능

    // ** input은 [value]를 이용해서 값을 얻어 오거나, 변경할 수 있므
...
function accessName(){
    ...
    for(let i=0; i<hobbyList.length;i++){
        ...
        // .checked: 해당 요소가 체크되어 있으면, trye, 아니면 false 반환
        if(hobbyList[i].checked){ // 현재 요소가 체크되어 있르면
